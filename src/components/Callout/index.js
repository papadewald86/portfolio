import React from 'react';
import clsx from 'clsx';

// Same colors and icons as Docusaurus's built-in admonitions, but icon-only
// -- no repeated "info"/"warning"/"tip" text heading.
const ICONS = {
  info: {
    viewBox: '0 0 14 16',
    path: 'M7 2.3c3.14 0 5.7 2.56 5.7 5.7s-2.56 5.7-5.7 5.7A5.71 5.71 0 0 1 1.3 8c0-3.14 2.56-5.7 5.7-5.7zM7 1C3.14 1 0 4.14 0 8s3.14 7 7 7 7-3.14 7-7-3.14-7-7-7zm1 3H6v5h2V4zm0 6H6v2h2v-2z',
  },
  warning: {
    viewBox: '0 0 16 16',
    path: 'M8.893 1.5c-.183-.31-.52-.5-.887-.5s-.703.19-.886.5L.138 13.499a.98.98 0 0 0 0 1.001c.193.31.53.501.886.501h13.964c.367 0 .704-.19.877-.5a1.03 1.03 0 0 0 .01-1.002L8.893 1.5zm.133 11.497H6.987v-2.003h2.039v2.003zm0-3.004H6.987V5.987h2.039v4.006z',
  },
  tip: {
    viewBox: '0 0 12 16',
    path: 'M6.5 0C3.48 0 1 2.19 1 5c0 .92.55 2.25 1 3 1.34 2.25 1.78 2.78 2 4v1h5v-1c.22-1.22.66-1.75 2-4 .45-.75 1-2.08 1-3 0-2.81-2.48-5-5.5-5zm3.64 7.48c-.25.44-.47.8-.67 1.11-.86 1.41-1.25 2.06-1.45 3.23-.02.05-.02.11-.02.17H5c0-.06 0-.13-.02-.17-.2-1.17-.59-1.83-1.45-3.23-.2-.31-.42-.67-.67-1.11C2.44 6.78 2 5.65 2 5c0-2.2 2.02-4 4.5-4 1.22 0 2.36.42 3.22 1.19C10.55 2.94 11 3.94 11 5c0 .66-.44 1.78-.86 2.48zM4 14h5c-.23 1.14-1.3 2-2.5 2s-2.27-.86-2.5-2z',
  },
};

// Docusaurus's admonition-to-Infima-alert-class mapping (tip -> success,
// info/warning/danger match their own names).
const ALERT_CLASS = {
  info: 'alert--info',
  warning: 'alert--warning',
  tip: 'alert--success',
  danger: 'alert--danger',
  note: 'alert--secondary',
};

export default function Callout({ type = 'info', children }) {
  const icon = ICONS[type] || ICONS.info;
  return (
    <div className={clsx('alert', ALERT_CLASS[type] || 'alert--info')} style={{ display: 'flex', gap: '0.6em', alignItems: 'flex-start' }}>
      <svg
        viewBox={icon.viewBox}
        style={{ width: '1.6em', height: '1.6em', flexShrink: 0, fill: 'var(--ifm-alert-foreground-color)', marginTop: '0.15em' }}
      >
        <path fillRule="evenodd" d={icon.path} />
      </svg>
      <div>{children}</div>
    </div>
  );
}
