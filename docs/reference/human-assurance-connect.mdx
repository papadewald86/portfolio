...
Batch **Human Assurance** verification through Connect: upload CSV to **`ToProve`**, Prove calls [`POST /v3/verify`](https://developer.prove.com/reference/verify) with **`verificationType`: `humanAssurance`**, and enrolls rows that pass the [Global Fraud Policy](https://developer.prove.com/reference/global-fraud-policy-3-1) in Identity Manager.

For realtime request and response semantics, see [Human Assurance Verify](https://developer.prove.com/how-to/human-assurance-verify). For continuous monitoring after enrollment, see [Human Assurance Manage](https://developer.prove.com/how-to/human-assurance-manage). For the Verified User batch contract, see [Verified User Connect](https://developer.prove.com/reference/verified-users-connect).

Connect is the Prove Platform **batch verification** interface. Customers upload CSV files over **SFTP**. Connect verifies each valid row with **[`POST /v3/verify`](https://developer.prove.com/reference/verify)**. Connect enrolls rows that pass the **[Global Fraud Policy](https://developer.prove.com/reference/global-fraud-policy-3-1)** in **Identity Manager**. Output columns align with synchronous verify responses. Interpret **`Authentication Result`**, **`Risk Result`**, and assurance fields using the [Global Fraud Policy](https://developer.prove.com/reference/global-fraud-policy-3-1) and your product verify guide.
...
| Verification | [`POST /v3/verify`](https://developer.prove.com/reference/verify) per valid detail row |
| Enrollment | Rows that pass the [Global Fraud Policy](https://developer.prove.com/reference/global-fraud-policy-3-1) enrolled in Identity Manager |
...
| Verification | Prove sends rows to **[`POST /v3/verify`](https://developer.prove.com/reference/verify)**. Response fields align with the live API. |
| Enrollment | Prove enrolls rows that pass the [Global Fraud Policy](https://developer.prove.com/reference/global-fraud-policy-3-1) in Identity Manager. |
...
Row-level verify failures still appear in the output file with **`Authentication Result`** / **`Risk Result`** and assurance fields populated from the verify response. See [Global Fraud Policy](https://developer.prove.com/reference/global-fraud-policy-3-1) for failure reason codes.
