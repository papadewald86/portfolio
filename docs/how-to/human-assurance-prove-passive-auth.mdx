---
title: "Human Assurance Prove Passive Authentication With Customer-Supplied Possession Fallback"
sidebar_label: "Prove Passive Authentication With Customer-Supplied Possession Fallback"
...
For client setup, see [Human Assurance Prove possession web](https://developer.prove.com/how-to/human-assurance-prove-possession-web-sdk), [Android](https://developer.prove.com/how-to/human-assurance-prove-possession-android-sdk), or [iOS](https://developer.prove.com/how-to/human-assurance-prove-possession-ios-sdk). For customer-only possession, see [Human Assurance customer-supplied possession with force bind](https://developer.prove.com/how-to/human-assurance-customer-possession). For product context, see [Human Assurance overview](https://developer.prove.com/explanation/human-assurance-overview).
...
- [`POST /v3/unify`](https://developer.prove.com/reference/unify-request)
- [`POST /v3/unify-status`](https://developer.prove.com/reference/unify-status-request)
- [`POST /v3/unify-bind`](https://developer.prove.com/reference/unify-bind-request)
:::

## Prerequisites

- **SDK and credentials**: Install a Prove server SDK and configure Sandbox OAuth. See [Set up Prove SDKs in your project](./dev-environment-sdk-setup).
- **Access token**: Obtain a bearer token using Prove OAuth. Use the same token for [`POST /v3/unify`](https://developer.prove.com/reference/unify-request), [`POST /v3/unify-status`](https://developer.prove.com/reference/unify-status-request), and [`POST /v3/unify-bind`](https://developer.prove.com/reference/unify-bind-request).
...
Send the phone number and **`possessionType=none`** to your back end, then call **`POST /v3/unify`**. See [`POST /v3/unify`](https://developer.prove.com/reference/unify-request) for optional identifiers such as **`clientCustomerId`**, **`clientRequestId`**, and **`checkReputation`**.
...
See [`POST /v3/unify-status`](https://developer.prove.com/reference/unify-status-request) for the full schema. Interpret **`evaluation`** using the [Global Fraud Policy](https://developer.prove.com/reference/global-fraud-policy-3-1).
...
Requires **`correlationId`** and **`phoneNumber`**. See [`POST /v3/unify-bind`](https://developer.prove.com/reference/unify-bind-request) for optional fields such as **`clientRequestId`**.
...
Return the binding outcome to the client. Interpret **`evaluation`** using the [Global Fraud Policy](https://developer.prove.com/reference/global-fraud-policy-3-1).
...
[`POST /v3/device/revoke`](https://developer.prove.com/reference/revoke-request) deactivates the Prove Key for a device. Pass the **`deviceId`** in the request body with a **`clientRequestId`**.
...
- **Access token**: Obtain a bearer token using Prove OAuth for [`POST /v3/unify`](https://developer.prove.com/reference/unify-request), [`POST /v3/unify-status`](https://developer.prove.com/reference/unify-status-request), and [`POST /v3/unify-bind`](https://developer.prove.com/reference/unify-bind-request).
...
4. **Verify mobile number** — Once the front end finishes the possession check, the back end calls [`POST /v3/unify-status`](https://developer.prove.com/reference/unify-status-request) with the correlation ID to validate the phone number. Expect `success=true`, `proveId`, `deviceId`, and `phoneNumber` from Mobile Auth in Sandbox. See [`POST /v3/unify-status`](https://developer.prove.com/reference/unify-status-request) for the full response.
...
4. **Verify mobile number** — Once the front end finishes the possession check, the back end calls [`POST /v3/unify-status`](https://developer.prove.com/reference/unify-status-request) with the correlation ID to validate the phone number. Expect `success=false` and `phoneNumber`. See [`POST /v3/unify-status`](https://developer.prove.com/reference/unify-status-request).
...
6. **Bind Prove Key** — The back end calls [`POST /v3/unify-bind`](https://developer.prove.com/reference/unify-bind-request) with the correlation ID. Expect `success=true`, `proveId`, and `phoneNumber` in Sandbox. See [`POST /v3/unify-bind`](https://developer.prove.com/reference/unify-bind-request) for the full response.
...
| --- | --- | --- |
| `401 Unauthorized` on Unify APIs | Invalid or expired bearer token | Request a fresh token from [`POST /token`](https://developer.prove.com/reference/authentication). |
